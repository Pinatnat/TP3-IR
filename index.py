...
class InvertedIndexReader(InvertedIndex):
    ...
    def get_postings_list(self, term):
        ...
        # TODO
        try :
            get_info = self.postings_dict[term]
            self.index_file.seek(get_info[0],0)
            get_data = self.index_file.read(get_info[2])
            posting_list = self.postings_encoding.decode(get_data)
            get_tf = self.index_file.read(get_info[3])
            tf = self.postings_encoding.decode_tf(get_tf)
            return (posting_list,tf)
        except KeyError:
            return None
        
    def get_avr_doc_length(self) :
        res = {}
        for a in self.doc_length.keys() :
            res[a] = self.doc_length[a]/len(self.doc_length)
        return res
...
class InvertedIndexWriter(InvertedIndex):
    ...
    def append(self, term, postings_list, tf_list):
        ...
        if len(self.postings_dict) == 0 :
            self.postings_dict[term] = (0,len(postings_list),len(encoded_postings_list), len(encoded_tf_list))
        else :
            start = self.postings_dict[last_term][0] + self.postings_dict[last_term][2] + self.postings_dict[last_term][3]  
            self.postings_dict[term] = (start,len(postings_list),len(encoded_postings_list), len(encoded_tf_list))
        self.index_file.write(encoded_postings_list)
        self.index_file.write(encoded_tf_list)
        for x in range(0, len(postings_list)) :
            try :
                self.doc_length[postings_list[x]]+=tf_list[x]
            except KeyError :
                self.doc_length[postings_list[x]]=tf_list[x]
...
if __name__ == "__main__":
    ...
    with InvertedIndexWriter('test', postings_encoding=VBEPostings, directory='./tmp/') as index:
        ...
        index.append(3, [6, 7, 9], [32, 12, 45, 3])
    
    with InvertedIndexReader('test', postings_encoding=VBEPostings, directory='./tmp/') as reader:
        print(reader.get_postings_list(1))
        print(reader.get_postings_list(2))
        print(reader.get_postings_list(3))
