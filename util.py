...
def sorted_merge_posts_and_tfs(posts_tfs1, posts_tfs2):
    ...
    b = 0
    new_list = []
    while a < len(posts_tfs1) or b < len(posts_tfs2) :
        if a < len(posts_tfs1) and b < len(posts_tfs2):
            if posts_tfs1[a][0] < posts_tfs2[b][0] :
                if a < len(posts_tfs1):
                    new_list.append(posts_tfs1[a])
                    a+=1
            elif posts_tfs1[a][0] > posts_tfs2[b][0] :
                if b < len(posts_tfs2) :
                    new_list.append(posts_tfs2[b])
                    b+=1
            elif posts_tfs1[a][0] == posts_tfs2[b][0] :
                new_list.append((posts_tfs1[a][0],posts_tfs1[a][1]+posts_tfs2[b][1]))
                a+=1
                b+=1
        else :
            if a >= len([posts_tfs1]) :
